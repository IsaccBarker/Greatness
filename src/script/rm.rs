...
pub fn rm(
    ...
    let specified_script = utils::relative_to_special(&utils::relative_to_script(manifest, &PathBuf::from(target_script)))?;
        ...
        for file in files {
            if file.path == specified_file {
                if let Some(scripts) = &mut file.scripts {
                    scripts.retain(|e| { println!("{} == {}", e.display(), &specified_script.display()); e != &specified_script } );
